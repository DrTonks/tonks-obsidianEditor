...
- **一键预览**：左侧书本图标或右键“预览”打开当前编辑内容的快照，再次切换返回编辑。
- **实时预览模式**：通过右键菜单、侧栏眼睛图标、命令面板或插件设置开启；点击预览块进入原位编辑，光标移出后恢复博客样式。保留原生选区、输入和撤销。
- **主题跟随博客**：预览与实时预览模式共用亮蓝、亮黄、暗蓝、暗黄四种主题。在插件设置的“预览主题”或整篇预览的主题选择器中切换，即时更新；实时预览的编辑区背景与文字同步，关闭后恢复 Obsidian 外观。
- **右键菜单**：预览、实时预览模式、博客特殊语法，分别用于整篇查看、原位编辑预览、插入博客格式。
...
普通“预览”仍是快照；新增的“实时预览模式”在停笔 300ms 后更新，未编辑的块保持博客渲染。首次使用需重建博客适配器，再构建、安装并重新启用本插件。开启时临时接管当前笔记的块渲染，关闭时恢复原编辑模式，不创建双栏或新窗口。新增博客格式先阅读 [博客适配器维护指南](https://github.com/DrTonks/tonks-blog/blob/main/editor/README.md)。
...
- 实时预览按完整 Markdown 块显示，光标所在段落、列表、表格或指令块保留源码；Frontmatter 与脚注定义保留原生编辑。链接跨块跳转等整篇交互请使用完整“预览”。
